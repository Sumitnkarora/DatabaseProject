CREATE TABLE [dbo].[stgCatalogImport](
	...
	[ProcessStatus] [bit] NOT NULL CONSTRAINT [DF_tblCatalogImport_Processed]  DEFAULT (0),
...
CREATE   TRIGGER [dbo].[trg_Insert_stgCatalogImport] 
ON [dbo].[stgCatalogImport]
AFTER INSERT 
AS
IF @@Rowcount = 0
    RETURN
...
BEGIN
	UPDATE a
	SET a.USDRetailPrice = a.ItemPrice
	FROM dbo.stgCatalogImport a
    INNER JOIN inserted cd
    ON a.PID = cd.PID
	INNER JOIN dbo.tblCatalogData c
    ON a.PID = c.PID
	WHERE a.USDRetailPrice IS NULL 
    AND  a.ItemPriceCurrency='USD' OR ( a.ItemPriceCurrency IS NULL AND c.ItemPriceCurrency = 'USD')
END
GO

GRANT SELECT, INSERT, UPDATE, DELETE ON [dbo].[stgCatalogImport] TO [CatalogImportApp];
GO