/* =============================================
 ...
 ============================================= */
CREATE PROCEDURE [dbo].[exporter_ProductAttributes_FinishSession]
...
BEGIN
	...
	BEGIN TRY
		...
		IF (@ExportStatusId = 2)
		BEGIN
			DELETE FROM FeedExportProductAttributes 
		...
		BEGIN
			...
			FROM FeedExportProductAttributes f
			WHERE f.SessionId = @SessionId AND f.StatusId = 1
		END
		...
		BEGIN
			...
			(SELECT i.ISBN13 FROM FeedExportProductAttributesInvalid i WITH (NOLOCK)
			WHERE i.SessionId = @SessionId
			GROUP BY i.ISBN13, i.EBookId) AS invalids;
		END
...
GRANT EXECUTE ON [dbo].[exporter_ProductAttributes_FinishSession] TO [FeedsExporter]; 
GO