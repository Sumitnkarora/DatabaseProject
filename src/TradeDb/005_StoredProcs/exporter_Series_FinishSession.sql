/* =============================================
 ...
 Revision history: Dec 7, 2017 Dariusz Holc
				   Add invalid record count. Fix catch block with proper casts.
				Feb 15, 2018 - Raymond Chan 
				Add NoLock when selecting count from invalid table
 ============================================= */
CREATE PROCEDURE [dbo].[exporter_Series_FinishSession]
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			(SELECT SeriesId FROM FeedExportSeriesInvalid WITH (NOLOCK)
			 WHERE SessionId = @SessionId
			 GROUP BY SeriesId, KoboSeriesId) AS Invalid;
		END
...
GRANT EXECUTE ON [dbo].[exporter_Series_FinishSession] TO [FeedsExporter]; 
GO